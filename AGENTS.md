...
- Core-loop smoke test: pick a blueprint (e.g. `Spire Block`) from the BUILD library, place it on a cell resting on the existing structure (gold decreases), then click `Start Wave` to enter the attack phase (wizard snaps to crown perch; enemies climb toward the solar collector; click empty cells to path the wizard, or hotbar+click to cast). Win by clearing a wave while framing height is still ≥ 100 (`docs/HEIGHT_PROGRESSION.md`). See `docs/PLAYER_MOVEMENT.md`.
