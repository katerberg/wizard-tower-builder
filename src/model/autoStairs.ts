...
function maxStructureRow(tower: Tower): number {
  let max = 0;
  for (const structure of tower.structures ?? []) {
    const top = structure.origin.row + structure.size.h - 1;
    if (top > max) max = top;
  }
  return max;
}

function maxStructureRowInCol(tower: Tower, col: number): number {
  let max = -1;
  for (const structure of tower.structures ?? []) {
    for (const c of roomCells(structure.origin, structure.size)) {
      if (c.col === col && c.row > max) max = c.row;
    }
  }
  return max;
}

function maxStructureRowInCols(tower: Tower, cols: number[]): number {
  let max = 0;
  for (const col of cols) {
    max = Math.max(max, maxStructureRowInCol(tower, col));
  }
  return max;
}

/** Highest row that needs stair infra (rooms and framing). */
function maxShaftRow(tower: Tower): number {
  return Math.max(maxRoomRow(tower), maxStructureRow(tower));
}
...
function assignShaftColumns(tower: Tower, mine?: MineState | null): number[] | null {
  ...
  const upTo = maxShaftRow(tower);
  for (let row = 0; row <= upTo; row++) {
    for (const seg of horizontalWalkableSegments(tower, row, mine)) {
      const alreadyServed = [...shafts].some((c) => seg.cols.includes(c));
      if (alreadyServed) continue;
      const hasFraming = seg.cols.some((col) => hasStructure(tower, col, row));
      if (!hasFraming) continue;
      const segmentUpTo = Math.max(row, maxStructureRowInCols(tower, seg.cols));
      const col = pickShaftColumn(tower, seg.cols, seg.cols[0], segmentUpTo);
      if (col === null) {
        console.warn(
          `Auto-stairs: no continuous shaft column for framing segment at row ${row}`,
        );
        return null;
      }
      shafts.add(col);
