...
describe('reconcileAutoStairs', () => {
  ...
  it('creates two shafts for disconnected wings (x x / x x / xxx)', () => {
    ...
    expect(placed.ok).toBe(true);
    tower = placed.tower!;
    // Right-wing framing gets its own shaft even before a room is placed there.
  ...
  it('serves the starter tower right wing with an auto shaft before rooms are placed there', () => {
    const tower = createStarterTower();
    // Twin-column starter: left rooms on col 6, right framing on col 8 — both need shafts.
    expect(stairCols(tower)).toEqual([6, 8]);

    const bp = getBlueprint('guardroomRoom')!;
    const placed = placeRoomReplacing(tower, createRoom('gR', bp, { col: 8, row: 1 }), bp);
    expect(placed.ok).toBe(true);
    expect(stairCols(placed.tower!)).toEqual([6, 8]);
  });
  ...
  it('removes orphan wing shaft when wing room is removed', () => {
    ...
    tower = {
      ...
      structures: (tower.structures ?? []).filter(
        (s) => !(s.origin.col === 6 && s.origin.row >= 1),
      ),
      structureOccupancy: Object.fromEntries(
        Object.entries(tower.structureOccupancy ?? {}).filter(
          ([key]) => Number(key.split(',')[0]) !== 6 || Number(key.split(',')[1]) === 0,
        ),
      ),
    };
