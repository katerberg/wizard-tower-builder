import type { ModificationDef } from './types';

const COSTS = [5, 8, 12];
export const SPIKES_DAMAGE_PER_LEVEL = 2;

/** Damages enemies each time they climb onto or past this room, or smash it. */
export const spikes: ModificationDef = {
  id: 'spikes',
  name: 'Spikes',
  glyph: '^',
  color: '#a0aec0',
  description: 'Punishes climbers who step on or beside this room, and anyone who attacks it.',
  mechanicsAtLevel: (level) =>
    `${SPIKES_DAMAGE_PER_LEVEL * level} damage per enemy step on/adjacent or room attack`,
  maxLevel: COSTS.length,
  cost: (level) => {
    const n = COSTS[level - 1];
    return n == null ? { stone: Infinity } : { stone: n };
  },
  onEnemyStep: {
    run: (ctx) => {
      if (!ctx.enemyTouchesFootprint) return;
      ctx.attackEnemy(ctx.enemy, SPIKES_DAMAGE_PER_LEVEL * ctx.level);
    },
  },
  onEnemyAttackRoom: {
    run: (ctx) => {
      ctx.attackEnemy(ctx.enemy, SPIKES_DAMAGE_PER_LEVEL * ctx.level);
    },
  },
};
